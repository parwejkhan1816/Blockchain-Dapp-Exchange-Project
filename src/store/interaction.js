...
export const loadNetwork = async (provider,dispatch) =>{
	...
	dispatch({type: 'NETWOTK_LOADED', chainId})
	return chainId;
}
...
export const loadBalances = async (exchange,tokens,account,dispatch)=>{
  ...
  balance = await ethers.utils.formatUnits(await exchange.balanceOf(tokens[0].address,account),18)
  dispatch({ type: 'EXCHANGE_TOKEN_1_BALANCE_LOADED' , balance})  

  balance = ethers.utils.formatUnits(await tokens[1].balanceOf(account),18);
  dispatch({ type: 'TOKEN_2_BALANCE_LOADED' , balance})

  balance = await ethers.utils.formatUnits(await exchange.balanceOf(tokens[1].address,account),18)
...
export const subscribeToEvents = (exchange,dispatch) =>{
	...
	exchange.on('Cancel',(id,user,tokenGet,amountGet,tokenGive,amountGive,timestamp,event) => {
		...
		dispatch({type: 'ORDER_CANCEL_REQUEST_SUCCESS', order, event })
	})
...
export const transferTokens = async (provider,exchange,transfertype,token,amount,dispatch)=>{
  ...
  try{
	...
	const amountToTransfer = await ethers.utils.parseUnits(amount.toString(),18);
